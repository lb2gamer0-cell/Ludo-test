...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // FIX: Render free-tier backend can take 30-50s to wake up from a cold
    // start. The old 8s abort timeout dropped that response almost every
    // time, and (depending on timing) could leave the screen stuck.
    // The timeout is now a safe 35s window, and the redirect to the game
    // now lives in a `finally` block so it ALWAYS fires exactly once -
    // whether the backend answers, errors out, or times out completely.
    const BACKEND_FETCH_TIMEOUT_MS = 35000; // safe window for Render cold start (8s -> 35s)

    async function completeUserLogin(userData) {
        // Save an optimistic local session immediately. app.js's auth guard
        // on game.html only checks localStorage, so the user is never
        // bounced back to index.html even if the network call below is slow.
        ...
        try {
            ...
            try {
                ...
                // Render cold start sometimes returns an HTML/proxy error
                // page instead of JSON while the container is waking up.
                console.warn('social-login: non-JSON response (server likely waking up):', parseErr.message);
            }
            ...
            if (data && data.success && data.user) {
                ...
                console.warn('social-login: backend returned status', res.status, '- continuing with local session.');
            }
        } catch (e) {
            // Covers network errors, CORS failures, and the 35s timeout/abort.
            // The local session was already saved above, so this never blocks login.
            console.log('social-login unreachable, continuing with local session:', e.message);
        } finally {
            // FIX: guaranteed, single redirect - runs no matter what happened
            // above, so the user can never be left stuck on the login screen.
    ...
    // FIX: On some Android WebViews / browsers with partitioned third-party
    // storage, Google's own consent/account-picker screen completes fine
    // ("email/alert" shows up) but `getRedirectResult()` comes back empty
    // even though Firebase has already restored the signed-in user. Before,
    // that meant handleSocialResult() was never called and the screen stayed
    // on the login page forever. `oauthHandled` guards against firing twice
    // when both listeners see the same login.
    ...
    if (auth) {
        ...
        // FIX: safety-net fallback. If Firebase confirms a signed-in user
        // but getRedirectResult() above missed it, and there is no local
